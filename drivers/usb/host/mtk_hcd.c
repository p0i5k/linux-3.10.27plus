...
#ifndef SUPPORT_WIRELESS_HCI_DUMP
	...
	void *dummy)
{
	...
	if (event == PM_SUSPEND_PREPARE) {
		...
		for(bPortNum=0; bPortNum< MUC_NUM_MAX_CONTROLLER; bPortNum++)
		{
			...
			if(bPortNum == 0)
				...
				if (!pController || !(pController->pThis))			// ADB pController->pThis will NULL
				 ...
				 printk("[Tandem] bPortInsertFlag = 0x%x  \n",bPortInsertFlag);
				}
			}
			bPortInsertFlag |= (((SONY_HUB_InPlugStatus & (1<<2))==0)?0:1)<< 1;
#else
		...
		printk("[Tandem] bPortInsertFlag = 0x%x  SONY_HUB_InPlugStatus = 0x%x \n",bPortInsertFlag,SONY_HUB_InPlugStatus);
		if(bPortInsertFlag)
		{
...
static int mgc_resume(struct device *dev)
{
...
#endif
	{
	    ...
	    if (diff == 1)
	    {
		pThis->MGC_pfPhyReset(pController->pBase, pController->pPhyBase, pController->pFMBase) ;
	    }
	    else
	    {
...
static void usb_events_timer_func (unsigned long _pThis)
{
    ...
    if(pThis->events & USB_EVENTS_HOST_SESSION_ON)
    {
        ...
        if (pThis->MGC_pfVbusControl)
        {
            ...
            mod_timer(&pThis->events_timer, jiffies + msecs_to_jiffies(4000));      
            pThis->events |= USB_EVENTS_VBUS_ON;
        }
...
static void MGC_WirelessEventDump(void)
{
    ...
    {
        printk("dump_index = 0x%x actual_length = 0x%x transfer_buffer_length = 0x%x type = %s\n",
                ...
                wireless_event_dump[event_index].type==1?"EVENT":
                wireless_event_dump[event_index].type==2?"ACL":"CMD");
...
#ifdef SUPPORT_WIRELESS_HCI_DUMP
    if(MGC_WIFIHotbootSupported(pUrb) && (pUrb->transfer_buffer_length != 0) && ((usb_pipein(pUrb->pipe) && ((uint8_t)usb_pipeendpoint(pUrb->pipe)==1)) || (usb_pipein(pUrb->pipe) && ((uint8_t)usb_pipeendpoint(pUrb->pipe)==2)) || ((usb_pipeout(pUrb->pipe)) && ((uint8_t)usb_pipeendpoint(pUrb->pipe)==0))))
    {
            ...
            for(i=0;i<HCI_DUMP_ENTRY_NUM;i++)
            {
                    ...
                    if(wireless_event_dump[i].dump_index == 0)
                    {
                            ...
                            if ( usb_pipein(pUrb->pipe) && ((uint8_t)usb_pipeendpoint(pUrb->pipe)==1) )
                                wireless_event_dump[i].type = 1;
                            else if ( usb_pipein(pUrb->pipe) && ((uint8_t)usb_pipeendpoint(pUrb->pipe)==2) )
                                wireless_event_dump[i].type = 2;
                            else
                                wireless_event_dump[i].type = 0;
    ...
    if (bXmt) /* transmit */
    {
        ...
        */
        ...
        if (bEnd)
        {
			...
			{
				//if(pUrb->transfer_buffer_length >= 2048)
					//INFO("[%d][WIFI EP%d]Set Toggle = %d/%d, length=%d.\n", mgc_togglecnt, pEnd->bRemoteEnd, mgc_togglecnt_prev, toggle, pUrb->transfer_buffer_length);
	...
	if (MGC_GetCurrentUrb(pEnd)== pUrb)
    {
        ...
        {                
            ...
            {                
				...
				if(MGC_WIFIHotbootSupported(pUrb) && (pEnd->bRemoteEnd == 8) && usb_pipebulk(pUrb->pipe)&&(usb_pipein(pUrb->pipe)== 0))
				{
					INFO("[WIFI EP%d]Set Toggle = %d/%d, length=%d.\n", pEnd->bRemoteEnd, mgc_toggle_prev, toggle, pUrb->transfer_buffer_length);
					if(mgc_toggle_prev == toggle)
					{
						//msleep(10);
						toggle = ((MGC_Read32(pBase, M_REG_TXDATATOG)) >> (pEnd->bEnd)) & 0x01;						
...
static irqreturn_t MUC_Irq(struct usb_hcd *hcd)
{
...
#endif
	{
	    ...
	    INFO("[USB]Re-Pull up RG_USB20_BGR_EN RG_USB20_CHP_EN on port1.\n");
...
static int MUC_hcd_probe(struct platform_device *pdev)
{
    ...
    if ((!pThis->MGC_pfPhyReset) ||
         (pThis->MGC_pfPhyReset(pController->pBase, pController->pPhyBase, pController->pFMBase) < 0))
    {
        return USB_ST_REMOVED;            
    }    
