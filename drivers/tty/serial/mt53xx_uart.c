...
static unsigned int _mt53xx_u5_INTEN_register_mask(unsigned int u4Reg);
...
#define MT53xx_PA_UART5     (0xF0000000 + 0x0c000)//(pPdwncIoMap + 0x880)//
#define MT53xx_VA_UART5     (0xF0000000 + 0x0c000)//(pPdwncIoMap + 0x880)//
#define MT53xx_PA_UART_IOMAP5 ((unsigned long)pUartIoMap + 0x0)
#define MT53xx_VA_UART_IOMAP5 (pUartIoMap + 0x0)
#endif
...
static unsigned int _mt53xx_u5_pdwnc_trans_mode_on(void)
{
    return UART_READ32(0x884) & U5_TRANSPARENT;
}
#endif
...
static void _mt53xx_u5__pdwnc_set_trans_mode(unsigned int on)
{
    if (on > 0)
    {
        UART_WRITE32(0xE2, 0x884);
    }
    else
    {
        UART_WRITE32(0, 0x884);
    }
}
...
#endif
{
    ...
    if (port == 0)
    {
		...
		#if defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5891)
		u4Tmp=_mt53xx_u5_INTEN_register_mask(u4Tmp);
		#endif
        u4Tmp &= ~U0_INTMSK;
    ...
    if (port == 5)
    {
        /* Change uartBT control to ARM mode,default is 8032 mode */
		//set 288a4[13]=1'b1
        u4Tmp = __raw_readl(pPdwncIoMap + 0x8a4);
        u4Tmp &= ~(0x2000);
		u4Tmp |= 0x2000; 
        __raw_writel(u4Tmp, pPdwncIoMap + 0x8a4);
		...
		// Set uart 5 initial settings
#ifndef ENABLE_UDMA
        // Disable modem control by default
        UART_WRITE32(0, 0x194);
#endif
		/*
		...
		*/
		//set uart BT pwdnc mode to trans mode
        if (!_mt53xx_u5_pdwnc_trans_mode_on())
        {
            _mt53xx_u5__pdwnc_set_trans_mode(1);
        }
...
#endif
        // Clear interrupt status
        u4Tmp = UART_READ32(UART5_INT_ID);
        u4Tmp &= ~(U5_INTMSK_R);
...
#if defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5891)
        || uCOMMCTRL == UART4_COMMCTRL //|| uCOMMCTRL == UART5_COMMCTRL (date sheet spec.)
#endif
       )
    {
        switch (baud)
...
static void _mt53xx_u5_int_enable(int enable)
{
    ...
    if (enable)
    {
		...
		//set 288a4[13]=1'b1
        unsigned int u4Reg;
        u4Reg = __raw_readl(pPdwncIoMap + 0x8a4);
		u4Reg &= ~(0x2000);
		u4Reg |= 0x2000; 
        __raw_writel(u4Reg, pPdwncIoMap + 0x8a4);
...
static irqreturn_t _mt53xx_uart5_interrupt(int irq, void* dev_id)
{
    ...
    /* rx mode */
    if (mport->fn_read_allow() || (uart_int_ident & (U5_RBUF_R | U5_RERR_R)))
    {
        _mt53xx_uart_rx_chars(mport);
        ret = IRQ_HANDLED;
    }
    ...
    /* tx mode */
    if (mport->fn_write_allow() || (uart_int_ident & U5_TBUF_R))
    {
        _mt53xx_uart_tx_chars(mport);
        ret = IRQ_HANDLED;
    }
...
static int _mt53xx_uart_suspend(struct platform_device* _dev, pm_message_t state)
{
    ...
    _mt53xx_uart_get(UART5_COMMCTRL, &_gUartSettings[4].i4BaudRate, &_gUartSettings[4].i4DataLen,
	 ...
	 /* Change uartBT control to 8032 mode */
	 _UART_HwWaitTxBufClear(UART_PORT_5); //rx must be Idle status
	  //set 288a4[13]=1'b0
	 {
	 	unsigned int u4Reg;
        u4Reg = __raw_readl(pPdwncIoMap + 0x8a4);
...
static int _mt53xx_uart_resume(struct platform_device* dev)
{
    ...
    _mt53xx_uart_set(UART5_COMMCTRL, _gUartSettings[4].i4BaudRate, _gUartSettings[4].i4DataLen,
	  ...
	  /* Change uartBT control to ARM mode,default is 8032 mode */
	  while ((UART_READ32(0x8B4) & UART_WRITE_ALLOW) != UART_WRITE_ALLOW); //rx must be Idle status (pdwnc 0xf00288b4 is status register)
	  //set 288a4[13]=1'b1
	  {
	 	unsigned int u4Reg;
        u4Reg = __raw_readl(pPdwncIoMap + 0x8a4);
...
static int __init _mt53xx_uart_init(void)
{
	...
	{ 
	  ...
	  //set 288a4[13]=1'b1
		...
		u4Reg |= 0x2000; 
        __raw_writel(u4Reg, pPdwncIoMap + 0x8a4);
		printk(KERN_DEBUG "Serial:  uart bt change to ARM mode 0x%p\n", (pPdwncIoMap + 0x8a4));
	}
