...
HS400_RETUNE_STEP2:
	...
	if(uTuneSepFlag == 0)
	{
		...
		for(i=0; i<8; i++){
			uDatDly_sep[i] = uDatDly_sep[i] - uDatDly;
...
{
	pr_err("---Kernel Set FIFO path and Init---\n");
	void __iomem *base = host->base;
	...
	sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_GET_CRC_MA, 1);/*[7]*/
	...
	// CMD_PAD_RDLY:
	sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, 15/*31*/);
	...
	// DAT_PAD_RDLY:
	sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_DATRRDLY, 15/*31*/);
