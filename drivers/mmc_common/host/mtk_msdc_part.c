...
//memset_alloc_pages();
	...
	//pr_err("[ZJ!!!] read partoffset = %llx, partsize = %llx | partno = %d, offset = %llx size = %llx\n", u8partoffs, u8partsize, partno, offset, size);
	...
	if (offset % 512)
	{
		...
		offset -= block_offset;
		//pr_err("ZJ!!! block_offset =%x offset = %x\n",block_offset, offset);
...
//memset_alloc_pages();
	...
	if (left_to_read >= 512)
	{
		...
		//pr_err("[ZJ]!!! offset=%x left_to_read =%x \n", offset, left_to_read);
	}
...
//memset_alloc_pages();
	...
	while (count < size + 4)
	{
		pr_err("||0x%x %x %x %x \n", *((u_char *)pvmem + count), *((u_char *)pvmem + count + 1), *((u_char *)pvmem + count + 2), *((u_char *)pvmem + count + 3));
		count += 4;
	}*/
...
int msdc_partwrite_rwaccess(int partno, uint64_t offset, uint64_t size, void *pvmem)
{
	...
	//pr_err("[ZJ!!!] write partoffset = %llx, partsize = %llx | partno = %d, offset = %llx size = %llx\n", u8partoffs, u8partsize, partno, offset, size);
	...
	if (left_to_write >= 512)
	{
		...
		left_to_write -= write_length;
		p_buffer += write_length;
		//pr_err("[ZJ]!!! offset=%x left_to_write =%x \n", offset, left_to_write);
	}
	...
	if (left_to_write)
	{
		...
		/*spin_unlock(&host->lock);
		u_char count = 0;
		pr_err("[ZJ!!!] Haha 4 write read data:\n");
		while (count < size)
		{
			pr_err("||0x%x %x %x %x \n", *((u_char *)pvmem + count), *((u_char *)pvmem + count + 1), *((u_char *)pvmem + count + 2), *((u_char *)pvmem + count + 3));
			count += 4;
		...
		//if (msdc_request(offset, 512, _blkpages, 0))
		if (msdc_request(offset, 512, _pu1blkbuf, 0))
		{
			goto write_fail;
		}
