...
int msdc_aes_decryption_sg(struct mmc_data *data,struct msdc_host *host)
{
	...
	for_each_sg(data->sg, sg, data->sg_len, i) {
		...
		dma_addr_t sgdmaddr = sg_dma_address(sg);
		unsigned long sgphy;

		len = sg->length;
		buff = (unsigned long)sg_virt(sg);
		sgphy = virt_to_phys((void *)buff);

		if (!sgdmaddr || (sgdmaddr >= 0x20000000L))
			ERR_MSG("sgdmaddr is 0x%lx, sgphy is 0x%lx, sg virt is 0x%lx."
					" sg num is %d/%d, sg length is 0x%x\n",
					sgdmaddr, sgphy, buff, i, data->sg_len, sg->length);

		page = sg_page(sg);
		offset = sg->offset;
		page = nth_page(page, (offset >> PAGE_SHIFT));
		if (PageHighMem(page))
			ERR_MSG("sg page is high mem, sg_virt is 0x%lx\n", buff);
...
static void msdc_set_mclk(struct msdc_host *host, msdc_state state, u32 hz)
{
	...
	if (state == MSDC_STATE_HS400 || state == MSDC_STATE_HS200) {
			if (host->u1HynixEmmcFlag){
				host->hw->clk_drv = 6;
				host->hw->cmd_drv = 7;
				host->hw->dat_drv = 6;
				pr_err("HNX Special 6 driv..\n");
			}
			else {
				host->hw->clk_drv = 5;
				host->hw->cmd_drv = 6;
...
static u32 msdc_abort_data(struct msdc_host *host)
{
	...
	while (state != 4) { // until status to "tran"
		...
		ERR_MSG("check card state<%d>,error status 0x%x \n", state,status);
		if (state == 5 || state == 6) {
...
static int msdc_pio_read(struct msdc_host *host, struct mmc_data *data)
{
	...
	ERR_MSG("PIO Read<%d>bytes\n", size); //to check log output
	if(data->error)
...
static int msdc_do_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (!data) {
		...
		cmd->resp[0],cmd->resp[1],cmd->resp[2],cmd->resp[3]);
			...
			//check if it's hynix emmc, need special driving for hs400
			if (0x90 == ((((u32)cmd->resp[0]) >> 24) & 0xFF))
			{
				pr_err("it's HNX, special driving needed\n");
				host->u1HynixEmmcFlag = 1;
...
static void MsdcAjustAutokTiming(struct msdc_host *host)
{
	#if 0
	//normally not need adjust TX Timing
	void __iomem *base = host->base;
	static int u4AutokTxTiming = 0;

	u4AutokTxTiming++;
	sdr_set_field(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT0_TXDLY, (u4AutokTxTiming%4));
	sdr_set_field(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT1_TXDLY, (u4AutokTxTiming%4));
	sdr_set_field(EMMC50_PAD_DAT23_TUNE, MSDC_EMMC50_PAD_DAT2_TXDLY, (u4AutokTxTiming%4));
	sdr_set_field(EMMC50_PAD_DAT23_TUNE, MSDC_EMMC50_PAD_DAT3_TXDLY, (u4AutokTxTiming%4));
	sdr_set_field(EMMC50_PAD_DAT45_TUNE, MSDC_EMMC50_PAD_DAT4_TXDLY, (u4AutokTxTiming%4));
	sdr_set_field(EMMC50_PAD_DAT45_TUNE, MSDC_EMMC50_PAD_DAT5_TXDLY, (u4AutokTxTiming%4));
	sdr_set_field(EMMC50_PAD_DAT67_TUNE, MSDC_EMMC50_PAD_DAT6_TXDLY, (u4AutokTxTiming%4));
	sdr_set_field(EMMC50_PAD_DAT67_TUNE, MSDC_EMMC50_PAD_DAT7_TXDLY, (u4AutokTxTiming%4));
	pr_err("Adjust AUTOK TX Timing DLY TO %d \n",(u4AutokTxTiming%4));
	#endif


	pr_err("adjust driving based clk %d, cmd %d, dat %d \n",host->hw->clk_drv,host->hw->cmd_drv,host->hw->dat_drv);
	host->hw->clk_drv += 1;
	host->hw->cmd_drv += 1;
	host->hw->dat_drv += 1;

	host->hw->clk_drv = (host->hw->clk_drv > 7) ?  7: host->hw->clk_drv;
	host->hw->cmd_drv = (host->hw->cmd_drv > 7) ?  7: host->hw->cmd_drv;
	host->hw->dat_drv = (host->hw->dat_drv > 7) ?  7: host->hw->dat_drv;
...
static int msdc_tune_cmdrsp(struct msdc_host *host)
{
	...
	if(cur_rsmpl >= 2){
		...
		sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, cur_rrdly & 31);
	}
	if (cur_rrdly >= 32 && sel) {
			cur_cntr = (orig_cmdrtc + 1) ;
		sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_CMD_RSP_TA_CNTR, cur_cntr & 7);
		}
	if (cur_cntr >= 8 && sel) {
	...
	sdr_get_field(MSDC_PATCH_BIT1, MSDC_PB1_CMD_RSP_TA_CNTR, orig_cmdrtc);
	sdr_get_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, orig_dl_cksel);
	INIT_MSG("TUNE_CMD: rsmpl<%d> rrdly<%d> cmdrtc<%d> dl_cksel<%d> sfreq.<%d>\n",
...
static int msdc_tune_read(struct msdc_host *host)
{
	...
	sdr_set_field(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, cur_dsmpl % 2);
...
static int msdc_tune_write(struct msdc_host *host)
{
	...
	sdr_set_field(MSDC_IOCON, MSDC_IOCON_DDLSEL, 1);
	cur_dsmpl = (orig_dsmpl + 1);
	sdr_set_field(MSDC_IOCON, MSDC_IOCON_W_D_SMPL, cur_dsmpl % 2);
...
static void msdc_config_mmc(struct mmc_host *mmc)
{
	struct msdc_host *host = mmc_priv(mmc);
		...
		pr_err("\n[msdc]Get 5891 ic ver 0x%x ,E1 %d, E2 %d \n",
					mt53xx_get_ic_version(),
					IS_IC_MT5891_ES1(),
					IS_IC_MT5891_ES2());
	...
	if (mt53xx_get_ic_version() > IC_VER_MT5891_AA)
	{
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	 ...
	 */
	mmc->parent->dma_mask = &mmc->parent->coherent_dma_mask;
#endif
