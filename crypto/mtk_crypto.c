        ...
        if (((u4VAddr & DCACHE_LINE_SIZE_MSK) != 0) || (((u4Size & DCACHE_LINE_SIZE_MSK) != 0)))
{
        ...
        src_addr = dma_map_single(NULL, (void *)u4VAddr, u4Size, DMA_BIDIRECTIONAL);
        fgRet = _TZ_CTL(u4Cmd, u4PAddr, u4Size);
        dma_unmap_single(NULL, src_addr, u4Size, DMA_BIDIRECTIONAL);
}
...
BOOL NAND_AES_Encryption(UINT32 u4InBufStart, UINT32 u4OutBufStart, UINT32 u4BufSize)
{
    ...
    src_addr = dma_map_single(NULL, phys_to_virt(u4InBufStart), u4BufSize, DMA_TO_DEVICE);
    dst_addr = dma_map_single(NULL, phys_to_virt(u4OutBufStart), u4BufSize, DMA_FROM_DEVICE);
...
INT32 GCPU_SHA_Cmd(UINT32 u4Handle, UINT32 u4Id, void* prParam, BOOL fgIOMMU)
{
    ...
    */
    ...
    if (u1PadFlag == 0)
    {
        ...
        src_addr = dma_map_single(NULL, phys_to_virt(rLPParam.u4SrcBufStart), rLPParam.u4DatLen, DMA_TO_DEVICE);
        fgRet = _GCPU_Hw_CmdExe(&rHwCmd);
        dma_unmap_single(NULL, src_addr, rLPParam.u4DatLen, DMA_TO_DEVICE);
    }
    else
    {
        ...
        src_addr = dma_map_single(NULL, phys_to_virt(rHwCmd.u4Param[12]), GCPU_SHA_1_PACKET_SIZE * 2, DMA_TO_DEVICE);
        fgRet = _GCPU_Hw_CmdExe(&rHwCmd);
        dma_unmap_single(NULL, src_addr, GCPU_SHA_1_PACKET_SIZE * 2, DMA_TO_DEVICE);
    }
...
BOOL SHA256_HW_TRANSFORM(UINT32 *state, UINT32 u4InBufStart, UINT32 u4BufferSize, BOOL fgFirstPacket)
{
	...
	src_addr = dma_map_single(NULL, phys_to_virt(u4InBufStart), u4BufferSize, DMA_TO_DEVICE);
	fgRet = _GCPU_Hw_CmdExe(&rHwCmd);
	dma_unmap_single(NULL, src_addr, u4BufferSize, DMA_TO_DEVICE);
