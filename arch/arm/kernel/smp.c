...
void __cpuinit __cpu_die(unsigned int cpu)
{
	...
	printk(KERN_NOTICE "CPU%u: shutdown-1\n", cpu);
...
static int __init smp_custom_ipi_init(void)
{
    ...
    /* alloc descs for these custom ipis/irqs before
     * using them */
    irq_alloc_descs(IPI_CUSTOM_FIRST, 0, IPI_CUSTOM_LAST - IPI_CUSTOM_FIRST + 1, 0);

    for (ipinr = IPI_CUSTOM_FIRST; ipinr <= IPI_CUSTOM_LAST; ipinr++) {
        irq_set_percpu_devid(ipinr);
        irq_set_chip_and_handler(ipinr, &custom_ipi_chip, handle_custom_ipi_irq);
