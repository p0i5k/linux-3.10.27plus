...
#else
#ifdef CC_S_PLATFORM
	printk(KERN_ALERT "Function entered at [<%08lx>] from [<%08lx>]\n", where, from);
#else
...
{
	dump_mem("", "Stack-128: ", regs->ARM_sp - 128, regs->ARM_sp);
	dump_mem("", "Stack: ", regs->ARM_sp, regs->ARM_sp + nbytes);
}
