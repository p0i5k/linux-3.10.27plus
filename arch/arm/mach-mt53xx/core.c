...
#define SONY_VERSION_LEN 1024
char sony_version[SONY_VERSION_LEN];

static int show_sony_version(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n", sony_version);
...
static int sony_version_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, show_sony_version, inode);
}

static int sony_version_proc_write(struct file *file, const char *buffer,
					size_t count, loff_t *pos)
{
	char buf[SONY_VERSION_LEN];
	unsigned long len = (count > SONY_VERSION_LEN - 1) ? SONY_VERSION_LEN - 1 : count;

        if (copy_from_user(buf, buffer, len))
		return -EFAULT;

	buf[len] = '\0';
	strcpy(sony_version, buf);

        return count;
}

static const struct file_operations sony_version_proc_fops = {
	.open		= sony_version_proc_open,
	.read		= seq_read,
	.write		= sony_version_proc_write,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
static void  __init mt53xx_init_machine(void)
{
  ...
  /*    proc_file = create_proc_entry("boottime", S_IFREG | S_IWUSR, NULL); */
    ...
    proc_create("sony_version", 0644, NULL, &sony_version_proc_fops);
#if 0
